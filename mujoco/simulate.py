...
while True:
    ...
    # r knee
    sim_state.qpos[13] = np.deg2rad(-38)

    # l hip
    sim_state.qpos[14] = np.deg2rad(2)
    sim_state.qpos[15] = np.deg2rad(1)
    ...
    # r shoulder
    sim_state.qpos[18] = np.deg2rad(-9)
    sim_state.qpos[19] = np.deg2rad(-13)

    # r elbow
    sim_state.qpos[20] = np.deg2rad(50)

    # l shoulder
    sim_state.qpos[21] = np.deg2rad(-6)
    sim_state.qpos[22] = np.deg2rad(-7)

    sim_state.qpos[23] = np.deg2rad(-56)
